...
const FilterModal = ({ visible, onClose, onApply, onClear, currentFilters, screen }) => {
  ...
  const toggleBedSelection = (num) => {
    setFilters(prev => {
      const currentBeds = prev.beds || [];
      
      // If selecting a number, remove all lower numbers and add this one
      // This creates the "X+" behavior where selecting 3+ means 3 or more bedrooms
      if (typeof num === 'number') {
        // Remove all numbers less than or equal to the selected number
        const filteredBeds = currentBeds.filter(bed => bed > num);
        // Add the selected number if not already present
        if (!filteredBeds.includes(num)) {
          filteredBeds.push(num);
        }
        return { ...prev, beds: filteredBeds };
      } else {
        // For "5+", just toggle it
        const newBeds = currentBeds.includes(num)
          ? currentBeds.filter(b => b !== num)
          : [...currentBeds, num];
  ...
  const toggleBathSelection = (num) => {
    setFilters(prev => {
      const currentBaths = prev.baths || [];
      
      // If selecting a number, remove all lower numbers and add this one
      // This creates the "X+" behavior where selecting 3+ means 3 or more bathrooms
      if (typeof num === 'number') {
        // Remove all numbers less than or equal to the selected number
        const filteredBaths = currentBaths.filter(bath => bath > num);
        // Add the selected number if not already present
        if (!filteredBaths.includes(num)) {
          filteredBaths.push(num);
        }
        return { ...prev, baths: filteredBaths };
      } else {
        // For "5+", just toggle it
        const newBaths = currentBaths.includes(num)
          ? currentBaths.filter(b => b !== num)
          : [...currentBaths, num];
        ...
        <View style={styles.modalContent}>
          ...
          <View style={styles.savedFiltersHeader}>
            ...
            {showSavedFilters && (
              <View style={styles.savedFiltersContainer}>
                {isLoading ? (
                  <ActivityIndicator size="small" color="#fc565b" style={{ marginVertical: 10 }} />
          ...
          <View style={styles.filterNameContainer}>
            <View style={styles.filterNameRow}>
              ...
              <Text style={styles.filterNameLabel}>Filter Name</Text>
            </View>
            <TextInput
            ...
            <Text style={styles.modalTitle}>Filters</Text>
              ...
              <View style={styles.filterSection}>
                ...
                <View style={styles.sliderContainer}>
                  <Text style={styles.sliderLabel}>Min Price: ${parseInt(filters.priceRange?.min || 0).toLocaleString()}</Text>
                  <Slider
                    style={styles.slider}
                    minimumValue={0}
                    maximumValue={2000000}
                    step={10000}
                    value={parseInt(filters.priceRange?.min || 0)}
                  ...
                  />
                  <Text style={styles.sliderLabel}>Max Price: ${parseInt(filters.priceRange?.max || 2000000).toLocaleString()}</Text>
                  <Slider
                    style={styles.slider}
                    minimumValue={0}
                    maximumValue={2000000}
                    step={10000}
                    value={parseInt(filters.priceRange?.max || 2000000)}
                    minimumTrackTintColor="#fc565b"
                    maximumTrackTintColor="#ddd"
                    thumbTintColor="#fc565b"
                    onValueChange={(value) => setFilters(prev => ({
              ...
              <View style={styles.filterSection}>
                ...
                <View style={styles.sliderContainer}>
                  <Text style={styles.sliderLabel}>Min: {parseInt(filters.sqft?.min || 0).toLocaleString()} sqft</Text>
                  <Slider
                    style={styles.slider}
                    minimumValue={0}
                    maximumValue={10000}
                    step={100}
                    value={parseInt(filters.sqft?.min || 0)}
                    minimumTrackTintColor="#fc565b"
                    maximumTrackTintColor="#ddd"
                    thumbTintColor="#fc565b"
                    onValueChange={(value) => setFilters(prev => ({
                  ...
                  />
                  <Text style={styles.sliderLabel}>Max: {parseInt(filters.sqft?.max || 10000).toLocaleString()} sqft</Text>
                  <Slider
                    style={styles.slider}
                    minimumValue={0}
                    maximumValue={10000}
                    step={100}
                    value={parseInt(filters.sqft?.max || 10000)}
                    minimumTrackTintColor="#fc565b"
                    maximumTrackTintColor="#ddd"
                    thumbTintColor="#fc565b"
                    onValueChange={(value) => setFilters(prev => ({
              ...
              <View style={styles.filterSection}>
                ...
                <View style={styles.sliderContainer}>
                  <Text style={styles.sliderLabel}>Min Year: {parseInt(filters.yearBuilt?.min || 1900)}</Text>
                  <Slider
                    style={styles.slider}
                    minimumValue={1900}
                    maximumValue={new Date().getFullYear()}
                    step={1}
                    value={parseInt(filters.yearBuilt?.min || 1900)}
                    minimumTrackTintColor="#fc565b"
                    maximumTrackTintColor="#ddd"
                    thumbTintColor="#fc565b"
                    onValueChange={(value) => setFilters(prev => ({
                  ...
                  />
                  <Text style={styles.sliderLabel}>Max Year: {parseInt(filters.yearBuilt?.max || new Date().getFullYear())}</Text>
                  <Slider
                    style={styles.slider}
                    minimumValue={1900}
                    maximumValue={new Date().getFullYear()}
                    step={1}
                    value={parseInt(filters.yearBuilt?.max || new Date().getFullYear())}
                    minimumTrackTintColor="#fc565b"
                    maximumTrackTintColor="#ddd"
                    thumbTintColor="#fc565b"
                    onValueChange={(value) => setFilters(prev => ({
...
const styles = StyleSheet.create({
  ...
  modalContent: {
    ...
    flex: 1,
    padding: 20,
    paddingTop: Platform.OS === 'ios' ? (StatusBar.currentHeight || 44) + 20 : 20,
    marginTop: 0,
  },
  modalTitle: {
    fontSize: 24,
  ...
  extraSpacing: {
    height: 100, // Add extra height for scrolling
  },
  filterNameRow: {
    flexDirection: 'row',
    alignItems: 'center',
