...
// Property preferences options (same as PersonalPreferencesScreen)
...
const ProfileScreen = ({ navigation }) => {
  ...
  const handleOpenHelpFeedback = () => {
    setFeedbackText('');
    setIsHelpFeedbackModalVisible(true);
  };
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      const feedbackData = {
        UserId: userId,
        UserName: `${userData?.Profile?.FirstName || ''} ${userData?.Profile?.LastName || ''}`.trim() || 'Anonymous',
      ...
      Alert.alert(
        ...
        [{ text: 'OK', onPress: () => setIsHelpFeedbackModalVisible(false) }]
      );
  ...
  const handlePreferenceToggle = (preference) => {
    if (editablePreferences.includes(preference)) {
      // Remove preference
      setEditablePreferences(editablePreferences.filter(item => item !== preference));
    } else if (editablePreferences.length < 5) {
      // Add preference
      setEditablePreferences([...editablePreferences, preference]);
    }
  };
  ...
  const handleSavePreferences = async () => {
    try {
      ...
      // Update location with coordinates - ensure coordinates are valid
      const locationData = {
      ...
      if (editableLocation.coordinates && 
          editableLocation.coordinates.latitude && 
          editableLocation.coordinates.longitude) {
        locationData.Coordinates = {
          latitude: editableLocation.coordinates.latitude,
          longitude: editableLocation.coordinates.longitude
        };
      }

      if (updatedData.Location) {
        if (Array.isArray(updatedData.Location)) {
          updatedData.Location[0] = locationData;
        } else {
        ...
        <View style={styles.sectionCard}>
          ...
          <View style={styles.preferencesGrid}>
            {propertyPreferences.map((preference, index) => {
              const isSelected = editablePreferences.includes(preference);
              return (
                <TouchableOpacity
                  key={index}
                  style={[
                    styles.preferenceGridItem,
                    isSelected && styles.selectedPreferenceGridItem
                  ]}
                  onPress={() => handlePreferenceToggle(preference)}
                  disabled={!isSelected && editablePreferences.length >= 5}
                >
                  <Text style={[
                    styles.preferenceGridText,
                    isSelected && styles.selectedPreferenceGridText
                  ]}>
                    {preference}
                  </Text>
                  {isSelected && (
                    <Ionicons name="checkmark-circle" size={20} color="#fff" style={styles.checkmarkContainer} />
                  )}
                </TouchableOpacity>
              );
            })}
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer} ref={formScrollViewRef}>
        ...
        <View style={styles.sectionContainer}>
          ...
          <View style={styles.cardContainer}>
            {renderOptionItem('help-circle-outline', 'Help and Feedback', handleOpenHelpFeedback)}
            {renderOptionItem('document-text-outline', 'Terms of Use', handleOpenTerms)}
          </View>
        </View>
        ...
        <Modal
          animationType="slide"
          transparent={true}
          visible={isHelpFeedbackModalVisible}
          onRequestClose={() => setIsHelpFeedbackModalVisible(false)}
        >
          <KeyboardAvoidingView 
            behavior={Platform.OS === "ios" ? "padding" : "height"}
            style={styles.modalContainer}
          >
            <View style={styles.preferencesModalContent}>
              <View style={styles.editModalHeader}>
                <Text style={styles.editModalTitle}>Help and Feedback</Text>
                <TouchableOpacity onPress={() => setIsHelpFeedbackModalVisible(false)}>
                  <Ionicons name="close" size={24} color="#333" />
                </TouchableOpacity>
              </View>
              ...
              <ScrollView style={styles.preferencesContainer}>
                <View style={styles.sectionCard}>
                  ...
                  <Text style={styles.cardSubtitle}>Need immediate assistance? Call our support team.</Text>
                  
                  <View style={styles.supportPhoneContainer}>
                    <Ionicons name="call" size={24} color="#fc565b" style={styles.supportIcon} />
                    <View style={styles.phoneInfo}>
                      <Text style={styles.phoneLabel}>Support Phone</Text>
                      <Text style={styles.phoneNumber}>801-616-6985</Text>
                    </View>
                    <TouchableOpacity 
                      style={styles.callButton}
                      onPress={() => Linking.openURL('tel:801-616-6985')}
                    >
                ...
                <View style={styles.sectionCard}>
                  <Text style={styles.cardTitle}>Submit Your Feedback</Text>
                  <Text style={styles.cardSubtitle}>We value your input to improve our app.</Text>
              ...
              <View style={styles.editModalFooter}>
                <TouchableOpacity 
                  style={styles.cancelButton} 
                  onPress={() => setIsHelpFeedbackModalVisible(false)}
                >
                  <Text style={styles.cancelButtonText}>Cancel</Text>
                </TouchableOpacity>
                {/* No save button for feedback, it's submitted immediately */}
              </View>
            </View>
          </KeyboardAvoidingView>
        </Modal>
        ...
        >
          <SafeAreaView style={styles.modalContainer}>
            <View style={styles.preferencesModalContent}>
              <View style={styles.editModalHeader}>
                <Text style={styles.editModalTitle}>Terms of Use</Text>
              ...
              <ScrollView style={styles.preferencesContainer}>
                <View style={styles.sectionCard}>
                  ...
                  <Text style={styles.cardSubtitle}>Please read our terms of use carefully.</Text>
              ...
              <View style={styles.editModalFooter}>
                ...
                {/* No save button for terms, it's a static document */}
              </View>
            </View>
          </SafeAreaView>
        </Modal>
        ...
        {/* App Version */}
        <Text style={styles.versionText}>Version {Constants.expoConfig?.version || '3.1.4'} ({Constants.expoConfig?.ios?.buildNumber || '1'})</Text>
      </ScrollView>
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  ...
  preferenceGridItem: {
    width: (width - 50) / 3, // Account for container padding and margins
    height: (width - 50) / 3,
    margin: 2,
    marginBottom: 8,
    borderRadius: 8,
  ...
  selectedPreferenceGridItem: {
    backgroundColor: '#fc565b',
  },
  preferenceGridText: {
    fontSize: Math.min((width - 40) / 3 * 0.2, 10),
    fontWeight: '500',
    color: '#333',
    textAlign: 'center',
    paddingHorizontal: 2,
    lineHeight: Math.min((width - 40) / 3 * 0.25, 12),
  },
  selectedPreferenceGridText: {
    color: '#fff',
  },
  checkmarkContainer: {
    position: 'absolute',
