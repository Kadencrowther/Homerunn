...
import { 
  ...
  Dimensions, Vibration, Platform, StatusBar, ScrollView, ActivityIndicator 
} from 'react-native';
...
const HomeScreen = () => {
    ...
    onEnd: (event) => {
      ...
      if (shouldSwipe) {
        ...
        translateX.value = withSpring(
          ...
          () => {
            runOnJS(handleSwipe)(currentIndex, direction);
            translateX.value = 0;
            translateY.value = 0;
  ...
  const loadInitialData = useCallback(async (filterOverride = null) => {
    try {
      setIsLoading(true);
      ...
      if (user) {
        try {
          ...
          currentUserMetric = await getUserMatchMetric(user.uid);
  ...
  return (
    <TouchableOpacity activeOpacity={1} onPress={handleInteraction} style={styles.container}>
      ...
      {/* Replace Swiper with new gesture-based implementation */}
      <View style={styles.swiperContainer}>
        {currentDeck.length > 0 && currentIndex < currentDeck.length ? (
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  navigation.navigate('PropertyImages', { 
                    property: currentDeck[currentIndex], 
                    sourceScreen: 'Home' 
                  });
                }}
                activeOpacity={1}
          ...
          <View style={styles.noMoreCards}>
            <Text style={styles.noMoreCardsText}>No more properties to show</Text>
          </View>
        )}
      </View>
