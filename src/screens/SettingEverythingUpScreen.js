    ...
    try {
      ...
      const waitForAuth = () => new Promise((resolve) => {
        const unsubscribe = auth.onAuthStateChanged(user => {
          if (user) {
            console.log("Auth state changed - user is ready:", user.uid);
            resolve(user);
            unsubscribe();
          }
        });
      ...
      console.log("Auth is ready, proceeding with user:", currentUser.uid);

      if (!currentUser || !currentUser.uid) {
        throw new Error('User is not fully initialized');
      }
      ...
      try {
        ...
        // Show success message and stop here - no navigation
        setStatusMessage('Setup complete! You can now use the app.');
        console.log('Setup complete - waiting for explicit navigation trigger');
        
        // We're purposely NOT navigating here to avoid premature navigation
        ...
        try {
          ...
          // Show success message and stop here - no navigation
          setStatusMessage('Setup complete after retry! You can now use the app.');
          console.log('Setup complete after retry - waiting for explicit navigation trigger');
