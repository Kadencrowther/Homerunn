...
const PropertyDetails = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchCompletePropertyData = async () => {
      try {
        ...
        // Get the property ID or MLS number to use for the API call
        const propertyId = initialProperty.id || initialProperty.ListingId || initialProperty.mlsNumber;
        ...
        console.log('Fetching complete property data for:', propertyId);
        
        // Call the API to get the complete property data
        const result = await fetchMLSData({ listingId: propertyId });
        
        if (result && result.properties && result.properties.length > 0) {
          // Get the first property from the result
          const completeProperty = result.properties[0];
          
          console.log('Received complete property data:', completeProperty);
          
          // Merge the initial property with the complete property data
          // This ensures we keep any fields that might be in the initial property but not in the API response
          setProperty({
            ...initialProperty,
            ...completeProperty
          });
  ...
  const mapPropertyData = () => {
    ...
    const mappedProperty = {
      ...
      // Add listing agent information
      listAgentName: property.ListAgentFullName || '-',
      listAgentPhone: property.ListAgentPreferredPhone || '-',
  ...
  // Alternative shimmer with static gradient
