...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, Modal, Image, ActivityIndicator, Platform, FlatList, Keyboard } from 'react-native';
    ...
    if (auth.currentUser) {
      try {
        ...
        if (userMatchMetricDoc.exists()) {
          ...
          if (!isCurrentlyLoved) {
            // If property is being loved, add to loved properties only
            if (!lovedProperties.includes(propertyId)) {
              lovedProperties = [...lovedProperties, propertyId];
            }
          } else {
            // If property is being unloved, just remove from loved properties
            lovedProperties = lovedProperties.filter(id => id !== propertyId);
          }
          
          // Update Firebase - only update LovedProperties
          await updateDoc(userMatchMetricRef, {
            LovedProperties: lovedProperties
          });
          
          console.log(`Property ${propertyId} ${!isCurrentlyLoved ? 'added to' : 'removed from'} loved properties in Firebase`);
        }
      } catch (error) {
