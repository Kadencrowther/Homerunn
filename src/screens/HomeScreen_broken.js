...
const HomeScreen = () => {
  ...
  const updateCardCategories = useCallback(() => {
    ...
    const onDeckCards = currentDeck.slice(currentIndex + 1, currentIndex + 1 + STABLE_DECK_SIZE);
    
    // Get the future cards (beyond the stable deck)
    const futureCards = currentDeck.slice(currentIndex + 1 + STABLE_DECK_SIZE);
  ...
  }, [loadMorePropertiesInBackground, userMatchMetric, setCurrentDeckWithProtection, updateCardCategories, STABLE_DECK_SIZE]);
